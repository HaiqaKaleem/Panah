...
def member(member_id="M1", member_type="beam", length=4.2, diameter=0.06):
    return DesignMember(
        id=member_id, type=member_type, material_id="MAT-01",
        length_m=length, diameter_m=diameter,
    )

def design(*members):
    return CanonicalDesignVersion(
        schema_version="1.0.0", design_type="roof_truss",
        version="DV-001", span_m=4.2, height_m=1.0,
        members=list(members), connections=[],
    )
...
def test_brace_conversion():
    result = member_to_primitive(member("M2", "brace", 2.3))
...
def test_builder_converts_all_members():
    result = build_geometry(design(member("M1"), member("M2", "brace", 2.3)))
...
def test_unsupported_member_type_fails():
    with pytest.raises(ValueError, match="Unsupported geometry member type"):
        member_to_primitive(member("M1", "column"))
...
def test_output_is_not_mesh_data():
    primitive = build_geometry(design(member("M1"))).primitives[0]
